...
- Test: `/usr/bin/node tests/filter.test.mjs && bash tests/installer.test.sh`
  — use the real node binary by absolute path; a PATH shim on this box
  redirects `node` to a remote build runner that requires a
  `package.json`/lockfile this repo doesn't have.
